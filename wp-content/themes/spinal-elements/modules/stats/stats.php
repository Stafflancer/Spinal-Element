...
wp_enqueue_style('stats_style', get_template_directory_uri() . '/static/css/modules/stats/stats.css', [], null);

wp_enqueue_script( 'stats_js', get_template_directory_uri() . '/static/js/modules/stats/stats.js', [], null, true );

if(have_rows('stats'))
{ ?>
    <section class="stats <?php if($add_pattern == 1){ echo "add-pattern"; } 
                ...
                <div class="stats-block">
                    <div class="stats-row"><?php
                        ...
                        { 
                            ...
                            $statn = get_sub_field('statn');
                            $suffix = get_sub_field('suffix');
                            $content = get_sub_field('content');  ?>
                            <div class="item wow fadeInDown">
                                <div class="stats-inner">
                                    <div class="stats-details">
                                        <div class="statsval"><?php 
                                            if (!empty($prefix)) 
                                            { ?>  
                                                <span class="prefix"><?php echo $prefix; ?></span><?php 
                                            } ?><?php 
                                            if (!empty($statn)) 
                                            { ?>
                                                <span class="stat count"><?php echo $statn; ?></span><?php 
                                            } ?><?php 
                                            if (!empty($suffix)) 
                                            { ?>
                                                <span class="suffix"><?php echo $suffix; ?></span><?php 
                                            } ?>  
                                        </div><?php 
                                        if (!empty($content)) 
                                        { ?>
